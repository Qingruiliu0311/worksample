...
const Card =({ userData, updateCardStatus }) => {
    ...
    const acceptFriendRequest = async () => {
        try {
            ...
            const response = await apiClient.post("friend/accept/", {
                card_id: userData.card_id, // Pass the pub_card ID in the request body
            });
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                updateCardStatus(userData.card_id, "accepted");
            } else {
                setNotification({
                    open: true,
    ...
    const declineFriendRequest = async () => {
        try {
            const response = await apiClient.post("friend/decline/", {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                updateCardStatus(userData.card_id, "declined");
            } else {
                setNotification({
                    open: true,
    ...
    const renderResponseCardEnd = () => {
        if (userData.friendship_status === "accepted") {
            return (
                <div className="accepted">
                    <p className="accept_text">Accepted</p>
                </div>
            );
        } else if (userData.friendship_status === "declined") {
            return (
                <div className="declined">
                    <p className="accept_text">Declined</p>
                </div>
            );
        } else {
            return (
                <div className="response_card_end">
                    <svg
    ...
    return (
        <div className="response_card">
            <div className="response_card_start">
                ...
                <div className="Response_card_info_div">
                    <p className="Response_card_info">Name: {userData.firstname} {userData.lastname}</p>
                    <p className="Response_card_info">Card ID: {userData.card_id}</p>
                </div>
            </div>

            {renderResponseCardEnd()}
